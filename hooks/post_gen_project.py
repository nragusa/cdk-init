...
if __name__ == '__main__':
    ...
    try:
        subprocess.run(['zsh', '-i', '-c', 'venv'], capture_output=True, text=True, check=True)
        subprocess.run(['uv', 'add', 'aws-cdk-lib'], capture_output=True, text=True, check=True)
        with open('.envrc', 'a') as f:
            f.write('export AWS_PROFILE=sso\n')
            f.write('export AWS_DEFAULT_REGION=us-east-2\n')
        subprocess.run(['direnv', 'allow'], capture_output=True, text=True, check=True)
